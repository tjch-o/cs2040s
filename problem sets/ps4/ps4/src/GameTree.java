...
public class GameTree {
    ...
    // leaf nodes are already initialised with their values
    // we just return the root node values
    int findValueHelper(TreeNode node, Player p) {
        // if you are a leaf just return your current value
        if (node.leaf) {
            return node.value;
        } else {
            if (p == Player.ONE) {
                // we find the maximum of all the children
                int max = Integer.MIN_VALUE;
                for (TreeNode child: node.children) {
                    /* as we walk down the tree the tree alternates between ONE 
                    and TWO */ 
                    max = Math.max(max, findValueHelper(child, Player.TWO));
                }
                // updating the value of the node
                node.value = max;
                return max;
            } else {
                // we find the minimum of all the children
                int min = Integer.MAX_VALUE;
                for (TreeNode child: node.children) {
                    min = Math.min(min, findValueHelper(child, Player.ONE));
                }
                node.value = min;
                return min;
            }
    ...
    int findValue() {
        if (root == null) {
            ...
            return findValueHelper(root, Player.ONE);
        }
    }
